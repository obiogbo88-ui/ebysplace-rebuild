                ...
                <div className="rounded-2xl border border-white/10 p-4" key={service.id}>
                  <div className="grid gap-4 md:grid-cols-[128px_1fr]">
                    ...
                    <div>
                      ...
                      <label className="mt-2 grid gap-1 text-xs uppercase tracking-[0.2em] text-primary/80">Service description<textarea defaultValue={service.description || ""} id={`service-description-${service.id}`} rows={3} /></label>
                      <div className="mt-3 grid gap-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
                        ...
                        <button className="btn-dark py-2" onClick={() => { const priceFrom = readAdminPrice(`price-${service.id}`, "Service price"); if (!priceFrom) return; const serviceDescription = (document.getElementById(`service-description-${service.id}`) as HTMLTextAreaElement).value.trim(); updateService.mutate({ id: service.id, priceFrom, duration: (document.getElementById(`duration-${service.id}`) as HTMLInputElement).value, isBookable: (document.getElementById(`availability-${service.id}`) as HTMLSelectElement).value as "true" | "false", description: serviceDescription.length >= 10 ? serviceDescription : undefined }); }}>Save service</button>
                      </div>
