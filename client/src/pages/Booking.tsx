...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    if (selectedService) set("serviceName", selectedService);
  }, []);
    ...
    <div className="luxury-shell">
      ...
      <main className="container section-pad">
        ...
        <p className="mt-4 max-w-3xl text-white/65">
          Choose any service from the full Eby’s Place catalogue, then select
          your date, time, client details, and address. The final step is a
          Stripe-hosted payment page for the exact £20 deposit.
        </p>
        <form onSubmit={submit} className="lux-card mt-10 grid gap-6">
          ...
          <div className="rounded-2xl border border-primary/30 bg-primary/10 p-5 text-primary">
            The next step opens Stripe Checkout for a £20 non-refundable
            deposit. This deposit secures the requested appointment and is
            clearly marked before payment.
          </div>
          <button
          ...
          {create.error && (
            <p className="text-destructive">{create.error.message}</p>
          )}
          {checkout.error && (
            <p className="text-destructive">{checkout.error.message}</p>
          )}
        </form>
      </main>
      <SiteFooter />
