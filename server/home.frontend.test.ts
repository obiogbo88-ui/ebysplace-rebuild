...
describe("Eby’s Place landing page visual refinements", () => {
  ...
  it("restores the previous homepage WhatsApp shortcut and includes the Back to top arrow button", () => {
    ...
    expect(appSource).toContain("function BackToTopButton()");
    expect(appSource).toContain('aria-label="Back to top"');
    expect(appSource).toContain('window.scrollTo({ top: 0, behavior: "smooth" })');
    expect(appSource).toContain("window.scrollY > 240");
    expect(appSource).toContain("bottom-24 left-4");
    expect(appSource).toContain("md:bottom-6 md:left-6");
    expect(appSource).toContain("<BackToTopButton />");
