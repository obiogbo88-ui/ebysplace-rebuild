...
describe("Eby’s Place landing page visual refinements", () => {
  ...
  it("uses the supplied stretched top logo and a reduced, aligned footer logo", () => {
    ...
    expect(homeSource).toContain("h-14 w-[8.75rem] object-contain object-left mix-blend-multiply min-[390px]:w-[9.5rem] sm:h-24 sm:w-[19rem] lg:h-20 lg:w-[16rem] xl:w-[18rem]");
    expect(homeSource).toContain('<div className="flex items-center">');
      expect(homeSource).toContain("btn-gold whitespace-nowrap px-2 py-1.5 text-[0.62rem] uppercase tracking-[.12em] min-[390px]:px-2.5 min-[390px]:text-[0.68rem] sm:px-4 sm:py-2.5 sm:text-sm sm:tracking-[.14em]");
      ...
      expect(homeSource).toContain('["/services", "/ai-try-on", "/braiders-near-me"].includes(item.href)');
      expect(homeSource).toContain('className="absolute inset-0 h-full w-full object-cover object-center lg:object-[center_20%]"');
